...
// @version      1.0.2
// @description  《也许同类型中最好用？》系列 - 显示 123 云盘身份信息为会员，支持在网盘与分享页面超过 1GB 上限下载文件（基于【123云盘下载辅助】修改），支持使用视频倍速、关闭广告、修改头像、修改用户名等功能，自定义程度超高！需要修改头部代码来配置显示内容（非常简单！）
	...
	unsafeWindow.XMLHttpRequest.prototype.setRequestHeader = function (header, value) {
		...
		if (!!url && url.includes('download')) {
			...
			if (resName.toLowerCase() in headers) {
				...
				if (user.debug === 1 || user.debug === 3) console.log("【123 云盘会员青春版】Hook XHR", "\n请求地址:", url, "\n请求头项:", { [oriResName]: oriResValue }, "\n修改头项:", { [resName]: resValue }, "\n随机项目:", info)
