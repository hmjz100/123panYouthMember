...
(function () {
	...
	*/
		...
		if (url.includes('api/user/info')) {
			...
			if (user.vip === 1) this.addEventListener('readystatechange', function () {
				if (this.readyState === 4) {
					...
					res.data.UserVipDetail.VipCode = user.svip ? 2 : 1 // SVIP
					...
					if (user.endtime) {
						...
						res.data.VipExpire = vipEndTime.toLocaleString() // 时间
						res.data.UserVipDetailInfos = res.data.UserVipDetail.UserVipDetailInfos = [
							...
							},
							{
								...
								IsUse: vipFeatureLaunchTime <= new Date()
							},
							{
								VipDesc: '青春创始包',
								TimeDesc: ` ${siteLaunchTime.toLocaleDateString()} 生效`,
								IsUse: siteLaunchTime <= new Date()
							}
						]
					}
	...
	waitForKeyElements(".header-tips", (element) => {
		...
		elemclone.text("感谢您使用会员青春版，好用的话就点击这儿来给个 Star 吧~");
