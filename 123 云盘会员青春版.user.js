...
(function () {
	...
	*/
		...
		if (url.includes('api/user/info')) {
			...
			if (user.vip === 1) this.addEventListener('readystatechange', function () {
				if (this.readyState === 4) {
					...
					res.data.vipType = res.data.vipSub = res.data.developSub = user.svip ? 2 : 1
				...
				if (this.readyState === 4 && this.status === 200) {
					...
					if (user.mail || user.phone || user.id) {
						...
						res = {
							...
							"message": "为了保证安全，修改任意一个关键数据（邮箱、手机号、ID）后，将不允许执行本操作（123 云盘会员青春版）",
							"data": null
						}
						if (user.debug === 1 || user.debug === 2) console.log("【123 云盘会员青春版】Hook XHR", "\n请求地址:", url, "\n原始回复:", oriRes, "\n修改回复:", res)
					}
			...
			this.addEventListener('readystatechange', function () {
				if (this.readyState === 4 && this.status === 200) {
					...
					if (user.mail || user.phone || user.id) {
						res = {
							"code": 400,
							"message": "为了确保安全，修改任意一个关键数据（邮箱、手机号、ID）后，将不允许执行本操作（123 云盘会员青春版）",
							"data": null
						}
						if (user.debug === 1 || user.debug === 2) console.log("【123 云盘会员青春版】Hook XHR", "\n请求地址:", url, "\n原始回复:", oriRes, "\n修改回复:", res)
					}
	...
	unsafeWindow.atob = function (input) {
		try {
			return originalAtob(decodeURIComponent(input));
		} catch (error) {
			return originalAtob(input);
		}
	};

	waitForKeyElements(".header-tips", (element) => {
		if (element.data("replaced")) return;
		let elemclone = element.clone();
		elemclone.text("感谢您使用本脚本，好用的话就点击这儿来给个 Star 吧~");
		elemclone.off("click").on("click", function () {
			window.open("https://github.com/hmjz100/123panYouthMember", "_blank");
		});
		elemclone.data("replaced", true);
		element.replaceWith(elemclone);
	})

	waitForKeyElements(".new-menu-item-image, .special-menu-item-container-migration--label, .sider-member-btn, .video-new-user-tips", (element) => {
		if (element.is(":hidden")) return;
		element.hide();
	})
	let allowedTexts = ["其他网盘数据转入", "同步空间", "回收站", "下载客户端"];
	waitForKeyElements(`.ant-menu.ant-menu-root.ant-menu-inline[role="menu"]`, (element) => {
		element.find(`[role="menuitem"]`).each(function () {
			const menuText = $(this).text().trim();
			if (!allowedTexts.includes(menuText) || $(this).is(":hidden")) return;
			$(this).hide();
		});
	})
	waitForKeyElements(".special-menu-item-container", (element) => {
		element.find(".special-menu-item-container-migration").each(function () {
			const menuText = $(this).text().trim();
			if (!allowedTexts.includes(menuText) || $(this).is(":hidden")) return;
			$(this).hide();
		});
	})
	waitForKeyElements(`.header-btn-list`, (element) => {
		element.find(`.btn-item`).each(function () {
			const menuText = $(this).text().trim();
			if (!allowedTexts.includes(menuText) || $(this).is(":hidden")) return;
			$(this).hide();
		});
	...
	function waitForKeyElements(select, action, once, iframeSelect) {
		var targets, found;
		if (typeof iframeSelect == "undefined")
			targets = $(select);
		else
			targets = $(iframeSelect).contents().find(select);
		...
		var controlKey = select.replace(/[^\w]/g, "_") + action.toString().replace(/[^\w]/g, "_");
		var timeControl = controlObj[controlKey];
		if (found && once && timeControl) {
			clearInterval(timeControl);
			delete controlObj[controlKey]
		} else {
			if (!timeControl) {
				timeControl = setInterval(() => {
					waitForKeyElements(select, action, once, iframeSelect);
				}, 1000);
